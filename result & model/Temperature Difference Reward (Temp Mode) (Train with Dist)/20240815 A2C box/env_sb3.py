...
class SB3Env(gym.Env):
    def __init__(self, broker_address, port, temp_topic, pwm_topic, target_temp_min, target_temp_max, algorithm, action_space_type='box', episode_max_steps=1000, render_mode='human', discrete_step=10, temp_config=None, dist_config=None):
        ...
        # 修改后的 self.observation_space 定义
        self.observation_space = spaces.Box(
            low=np.array([
                self.temp_config['target_temp_low'], 
                self.temp_config['actual_temp_low'], 
                self.temp_config['cooling_temp_low'], 
                0  # dist_dc 的下界
            ], dtype=np.float32),
            high=np.array([
                self.temp_config['target_temp_high'], 
                self.temp_config['actual_temp_high'], 
                self.temp_config['cooling_temp_high'], 
                self.dist_config['dist_max'] if self.dist_config else 0  # dist_dc 的上界
            ], dtype=np.float32)
        )
    ...
    def reset(self, seed=None, **kwargs):
        ...
        # 修改后的 reset 方法返回值
        return np.array([self.target_temp, self.actual_temp, self.cooling_temp, self.dist_dc]), {}
    ...
    def step(self, action):
        ...
        observation = np.array([self.target_temp, self.actual_temp, self.cooling_temp, self.dist_dc])

        # 获取时间戳
        self.unique_id = int(time.time() * 1000)
        
        # print(f"Check value before step print: unique_id={self.unique_id}, current_step={self.current_step}, target_temp={self.target_temp}, actual_temp={self.actual_temp}, cooling_temp={self.cooling_temp}, action_value={action}, heat_dc={self.heat_dc}, cool_dc={self.cool_dc}, dist_dc={self.dist_dc}, last_reward={self.last_reward}, done={done}")
