...
def create_action_noise(noise_config, n_actions):
    if noise_config['type'] == 'OrnsteinUhlenbeckActionNoise':
        mean = np.full(n_actions, noise_config['mean'])
        sigma = np.full(n_actions, noise_config['sigma'])
...
def run_a2c(config):
    ...
    try:
        # 创建环境实例
        env = create_sb3_env(general_config, a2c_config, 'A2C', action_space_type, temp_config, dist_config, is_eval=True)
...
def run_ddpg(config):
    ...
    try:
        ...
        action_noise = create_action_noise(ddpg_config['action_noise'], n_actions)
